...
import { getPrimaryCategory, getSecondaryCategory } from "@/lib/helpers";
...
export function BlocksNav() {
  ...
  const renderSecondaryCategory = (
    category: BlockSecondaryCategory,
    parentSlug: string,
    index: number,
  ) => {
    const categorySlug = `${parentSlug}/${category.slug}`;
    ...
    return (
      <Link
        ...
        className={cn(
          ...
          !category.published &&
            "hover:text-muted-foreground hover:border-transparent opacity-90",
        )}
      >
  ...
  const renderNav = (categories: BlocksConfig) => {
    return categories
      .map((category, index) => {
        ...
        const categoryElement = renderPrimaryCategory(
          category,
          categorySlug,
          index,
        );
