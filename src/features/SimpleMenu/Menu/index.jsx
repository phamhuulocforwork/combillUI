...
const Menu = ({ bgColor, shapeColor }) => {
  return (
    <div
      className={`flex-col sm:flex-row flex justify-between items-center gap-16 text-slate-50 bg-${bgColor} py-6 px-8 rounded-2xl shadow-md font-semibold`}
    >
      <div>
        <svg
      ...
      <ul className="flex-col sm:flex-row flex gap-14 justify-center items-center">
        <li className="hover:text-slate-400 transition-colors duration-300">
          <a href="#">Product</a>
        </li>
        <li className="hover:text-slate-400 transition-colors duration-300">
          <a href="#">Docs</a>
      ...
      <button
        className={`bg-${shapeColor} border border-${bgColor} py-2 px-6 rounded-full shadow-xl hover:animate-[spin_200ms_linear_infinite]`}
      >
        Get in touch
      </button>
    </div>
