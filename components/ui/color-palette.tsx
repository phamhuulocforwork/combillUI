      ...
      <div ref={ref} className={cn("flex flex-col gap-2", className)} {...props}>
        ...
        <div className="flex items-center gap-2">
          <div className="grid flex-1 grid-cols-6 gap-1 md:grid-cols-12">
            {colorObj.DEFAULT && <ColorShade hex={colorObj.DEFAULT} isDefault />}
...
const ColorPaletteDemo = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => {
    return (
      <div
        ref={ref}
      ...
      >
        {colors.map(([colorName, colorValue]) => {
          ...
          return <ColorRow key={colorName} colorName={colorName} colorObj={colorObj} />;
        })}
      </div>
    );
  }
